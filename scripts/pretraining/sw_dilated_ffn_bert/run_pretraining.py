...
def parse_args():
    ...
    parser.add_argument('--random_next_sentence', action='store_true',
                        help='Whether to do sentence order prediction'
                             'Effective only if --raw is set.')
    ...
    while step_num < num_steps:
        for _ in range(num_accumulated):
            ...
            for sample, ctx in zip(sample_l, ctx_l):
                ...
                #dilation = mx.np.ones((12,), dtype=np.int64, ctx=ctx)
                dilation = mx.np.array([4, 4, 4, 3, 3, 3, 2, 2, 2, 1, 1, 1],
                                       ctx=ctx, dtype=np.int64)

                with mx.autograd.record():
                    _, _, nsp_score, mlm_scores = model(input_id, segment_id,
                        valid_length, masked_position, dilation)
        ...
        step_num += 1
        # saving
        if step_num > 499999 and (step_num % save_interval == 0 or step_num >= num_steps):
